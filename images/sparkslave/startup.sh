...
#export SPARK_LOCAL_IP=sparkslave
...
#export SPARK_WORKER_OPTS="${SPARK_WORKER_OPTS} -Dspark.httpBroadcast.uri=http://${SPARK_MASTER_IP}:${SPARK_BROADCAST_PORT}"
...
#echo "spark.httpBroadcast.uri http://${SPARK_MASTER_IP}:${SPARK_BROADCAST_PORT}" >> ${SPARK_HOME}/conf/spark-defaults.conf
#echo "spark.repl.class.uri http://${SPARK_MASTER_IP}:${SPARK_REPLCLASSSERVER_PORT}" >> ${SPARK_HOME}/conf/spark-defaults.conf
